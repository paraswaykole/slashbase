...
export const createTab = createAsyncThunk(
    ...
    async (payload: { dbConnId: string, tabType: TabType, metadata?: any | undefined }, { rejectWithValue, getState }: any) => {
        ...
        const result = await apiService.createTab(dbConnectionId, tabType, mSchema, mName, queryId, query)
        if (result.success) {
            return {
                tab: result.data,
...
export const updateActiveTab = createAsyncThunk(
    ...
    async (payload: { tabType: TabType, metadata: Object }, { getState, rejectWithValue }: any) => {
        ...
        const result = await apiService.updateTab(dbConnection.id, String(activeTabId), tabType, metadata)
        if (result.success) {
            return {
                tab: result.data,
            }
